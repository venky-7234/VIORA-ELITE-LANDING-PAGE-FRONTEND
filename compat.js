...
const assignApplication=async(req,res)=>{
  const adminId=req.body.adminIdentifier??req.body.adminId??req.body.admin_id;
  ...
  const admin=await pool.query(`SELECT id FROM users WHERE id=$1 AND role IN ('ADMIN', 'SUPER_ADMIN', 'MANAGER') AND status='ACTIVE'`,[adminId]); if(!admin.rows[0])return fail(res,400,"Selected user must be an active Admin");
